...
### 1. 运行程序
双击 `service_manager_gui.exe` 启动程序。

### 2. 管理服务
*   **添加服务**：点击"添加服务"，填写服务名称、类型、脚本路径、工作目录等信息。
...
### 3. 系统服务模式（推荐）
若需要服务开机自启且在后台长期运行：
1.  在 GUI 底部点击 **"注册服务"**（需要管理员权限）。
2.  成功后，Windows 服务列表中会出现名为 `Service Manager Tool` 的服务。
3.  您可以手动启动该服务，或重启电脑使其自动运行。
4.  **注意**：系统服务模式下，子服务在后台运行，无可见窗口。您仍可打开 GUI 查看状态和管理配置。

### 4. 查看日志
右键点击服务列表中的某一项，选择 **"查看日志"** 即可查看该服务的运行日志。日志文件保存在程序目录下的 `logs` 文件夹中。
...
## 配置文件
*   服务配置存储在 `services_config.ini` 文件中。
*   运行状态（PID）存储在 `pids` 文件夹中，文件名格式为 `<服务名>.pid`。
*   日志文件存储在 `logs` 文件夹中，文件名格式为 `<服务名>.log`。

## 注意事项
*   **中文支持**：程序完全支持中文路径和中文服务名，所有文件操作使用Unicode编码。
*   **路径支持**：程序支持相对路径和绝对路径。相对路径会相对于程序所在目录进行解析，即使在系统服务模式下也能正常工作。
*   **Python 环境**：如果是 Python 服务，建议指定明确的 Python 解释器路径，或者确保全局 `python` 命令可用。
*   **权限**：注册和注销系统服务需要管理员权限。如果操作失败，请尝试以管理员身份运行程序。
*   **任务栏行为**：点击任务栏图标会将窗口切换到最前台，而不是最小化窗口。

## 更新日志

### v1.2 (2025-11-24)
*   ✅ 修复了点击任务栏图标导致程序最小化的问题，现在会切换到最前台
*   ✅ 完全支持中文路径和中文服务名
*   ✅ 修复了日志文件和PID文件名中文乱码问题
*   ✅ 修复了配置文件无法在中文路径下读取的问题
*   ✅ 所有文件操作改用Unicode API，提升兼容性
*   ✅ 添加了隐藏控制台功能
*   ✅ 在服务列表中显示"隐藏控制台"状态
